...
| | P16-02 | 增长窗口 ASR 轻量化：`beam_size=1`、关闭 temperature fallback，只做快速扫描（精度不重要，用于部分结果展示） | P16-01 完成 | ✅ 已完成 |
| **噪声早停** | P16-03 | temperature 重试早停：全段 ASR 使用 `temperature=[0.0]` 禁用 6x 回退循环，beam_size=5 的 beam search 已提供足够精度 | 语音链路打通 | ✅ 已完成 |
